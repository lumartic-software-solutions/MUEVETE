odoo.define('member_barcode_scanner.kiosk_mode', function (require) {
"use strict";
...
var ProxyDevice1  = core.Class.extend(core.mixins.PropertiesMixin,{
...
var MemberKioskMode = Widget.extend(BarcodeHandlerMixin, {
    events: {
        "click .o_hr_attendance_button_members": function(){ this.do_action('member_barcode_scanner.member_attendance_action_kanban'); },
        'keypress #member_barcode' : 'on_manual_scan',
    },
    ...
    init: function (parent, action) {
        ...
        res_company.query(['name', 'ip_address', 'port', 'receipt_to_printer'])
           .filter([['id', '=', Session.company_id]])
           .all()
           .then(function (companies){
                self.printer_ip = companies[0].ip_address;
                self.printer_port = companies[0].port;
                self.receipt_to_printer = companies[0].receipt_to_printer;
                if (self.receipt_to_printer){
                    self.proxy_device = new ProxyDevice1(self);
                }
    ...
    start: function () {
        ...
        this.notification_manager.appendTo($('#member_barcode'));
        var res_company = new Model('res.company');
        res_company.query(['name'])
           ...
           .then(function (companies){
                ...
                self.$el.html(QWeb.render("MemberAttendanceKioskMode", {widget: self}));
                self.start_clock();
            });
...
//                self.do_warn(_("Success"),_("Thanks for coming,Please collect your ticket."));
			...
			console.log(">>>>>Sssssssssssssssssssss",report)
...
//            else if(result.allowEvent){
//                self.do_warn(_("Success"),_("Thanks for coming"));
...
core.action_registry.add('member_attendance_kiosk_mode', MemberKioskMode);

return {
    MemberKioskMode: MemberKioskMode,
    Success: Success,
